...
const ContactInfo = () => {
  ...
  // animated text
  const animatedTitle = useTypewriter("Let’s Work Together!", 60, startTyping);
  const animatedSubtitle = useTypewriter(
    "I’m open to collaborations, freelance opportunities, or full-time roles. Feel free to reach out — I’d love to hear from you!",
    20,
    startTyping
  );
  return (
    <section ref={sectionRef} className="flex justify-center items-center">
      <GlassCard className="w-full max-w-4xl p-10 rounded-2xl shadow-xl bg-white/10 dark:bg-white/5 backdrop-blur-2xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-10">
          ...
          <div className="space-y-4 md:w-1/2">
            <h2 className="text-4xl md:text-5xl font-bold ">{animatedTitle}</h2>
          ...
          <div className="flex flex-col space-y-4 md:w-1/2 font-mono">
            {contactItems.map((item, index) => (
              <a
                ...
                className={`
                      flex items-center space-x-4 p-3 rounded-xl transition-all duration-500 
                      hover:translate-x-1
                      ${
                        animateList
                    ...
                    `}
                style={{ transitionDelay: `${index * 120}ms` }} // stagger effect
              >
